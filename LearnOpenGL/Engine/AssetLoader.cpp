...
MeshModel* AssetLoader::LoadMeshModelFromFile(const std::string& path) {
	for (MeshModel* mm : AssetManager::Get().GetAssets<MeshModel>()) {
		if (mm->assetPath == path) {
			return mm;
		}
	}
...
void AssetLoader::ProcessObjNode(aiNode* node, const aiScene* scene, const std::string& path, MeshModel* meshModel) {
	for (uint32_t i = 0; i < node->mNumMeshes; i++) {
		...
		StaticMesh* mesh = ProcessMesh(aimesh, scene, path);
...
StaticMesh* AssetLoader::ProcessMesh(aiMesh* mesh, const aiScene* scene, const std::string& path) {
...
std::vector<Texture*> AssetLoader::LoadTexturesFromMaterial(aiMaterial* mat, aiTextureType aiType, const std::string& path) {
...
Texture* AssetLoader::LoadTextureFromFile(const std::string& path) {
	...
	if (newTexture) {
		...
		OpenGL::Get().BufferTexture(newTexture);
